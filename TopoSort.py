...
#  Description: The following program determines if a given graph has a cycle and subsequently completes a topological sort on that graph if applicable.
...
class Graph(object):
    ...
    # The following function tests whether the given graph contains a cycle
    def has_cycle(self):

        cyclic = False # Initially set cyclic to False

        for v in range(len(self.Vertices)): # Using the cyclic variable within the dfs function, check every vertex
            
            if self.dfs(v): # If the dfs function is terminated early and returns cyclic to be true, this function will also return true; otherwise, false is returned
    ...
    # The function below is used as a helper function for topological sort.
    def topologicalSortUtil(self, v, stack):

        self.Vertices[v].visited = True # Mark the current node as visited

        for i in range(len(self.adjMat[v])): # Traverse through all indices of vertices adjacent to this vertex
            
            if self.adjMat[v][i] and self.Vertices[i].visited == False: # If the following compound condition is true, recursively call function with the index as an input
                self.topologicalSortUtil(i, stack) # Include stack as an input as well

        stack.append(self.Vertices[v].label) # Append label to the stack that stores results

    # Below is the function to do topological sort. It uses the helper function above.
    def toposort(self):

        stack = [] # Instantiate stack as empty list

        for v in range(len(self.Vertices)): # Traverse through all indices of vertices
            
            if self.Vertices[v].visited == False: # If vertex has not been visited, call the recursive helper function to store topological sort
                self.topologicalSortUtil(v, stack) # Also, include stack as an input

        return (stack[::-1])  # Return stack in reverse order
